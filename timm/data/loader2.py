...
def mp_distributed_training(dp, num_workers: int, batch_size: int, transform, rank: int, world_size: int, q):
    dist.init_process_group("gloo", rank=rank, world_size=world_size)
    dl = create_loader2(dp, batch_size=batch_size, num_workers=num_workers, img_transform=transform, distributed=True)
    cnt = 0
    for d in dl:
        cnt += 1
    ...
    if distributed:
        ...
        for rank in range(WORLD_SIZE):
            ...
            p = ctx.Process(target=mp_distributed_training,
                            args=(dp, num_workers, batch_size, transform, rank, WORLD_SIZE, q))
            pqs.append((p, q))
            p.start()
        ...
        dl = create_loader2(dp, batch_size=batch_size, num_workers=num_workers, img_transform=transform)
        for batch in dl:
            print(batch)
